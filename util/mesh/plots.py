...
def mesh_append(p, v, f=None, n=None, strategy='mesh', grid_on=False, clr='lightcoral',
                ...
                normal_scale=1):
    # Align arrays:
    v = v.np() if torch.is_tensor(v) else v
    f = f.np() if torch.is_tensor(f) else f
    n = n.np() if torch.is_tensor(n) else n
    clr = clr.np() if torch.is_tensor(clr) else clr
    normal_clr = normal_clr.np() if torch.is_tensor(normal_clr) else normal_clr
